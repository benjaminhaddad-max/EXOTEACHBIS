...
export default function ExamWorkflowStepper({
  ...
  function renderStep3() {
    return (
      <div className="space-y-4">
        <p className="text-sm text-white/60">
          Configurez les informations du sujet d'examen et générez le PDF formaté.
        </p>

        <div className="grid grid-cols-2 gap-3">
          <FormField
            label="Institution"
            value={pdfForm.institution}
            onChange={(v) => setPdfForm((p) => ({ ...p, institution: v }))}
          />
          <FormField
            label="Année universitaire"
            value={pdfForm.academicYear}
            onChange={(v) => setPdfForm((p) => ({ ...p, academicYear: v }))}
          />
          <FormField
            label="Titre de l'épreuve"
            value={pdfForm.examTitle}
            onChange={(v) => setPdfForm((p) => ({ ...p, examTitle: v }))}
            className="col-span-2"
          />
          <FormField
            label="Code UE"
            value={pdfForm.ueCode}
            onChange={(v) => setPdfForm((p) => ({ ...p, ueCode: v }))}
          />
          <FormField
            label="Nom de la matière"
            value={pdfForm.subjectName}
            onChange={(v) => setPdfForm((p) => ({ ...p, subjectName: v }))}
          />
          <FormField
            label="Durée"
            value={pdfForm.duration}
            onChange={(v) => setPdfForm((p) => ({ ...p, duration: v }))}
          />
          <FormField
            label="Date et heure"
            value={pdfForm.dateTime}
            onChange={(v) => setPdfForm((p) => ({ ...p, dateTime: v }))}
          />
        </div>
        ...
        {!pdfUrl && (
          <button
            ...
            className="flex items-center gap-2 rounded-lg bg-[#C9A84C] px-4 py-2 text-sm font-medium text-black transition hover:bg-[#d4b55c] disabled:opacity-50"
          >
            {generatingPdf ? (
  ...
  function renderStep4() {
    const effectiveCount =
      questionCount || importResult?.questionsCreated || 0;

    return (
      <div className="space-y-4">
        <p className="text-sm text-white/60">
          Générez la grille de réponses pour l'examen ou importez une grille personnalisée.
        </p>

        <div className="rounded-lg border border-white/10 bg-white/5 px-4 py-3 text-sm text-white/60">
          <FileSpreadsheet size={16} className="mb-1 mr-2 inline text-[#C9A84C]" />
          {effectiveCount} questions dans cette série
        </div>
        ...
        {!gridUrl && (
          <div className="flex gap-2">
            ...
            >
              ...
              {generatingGrid ? "Génération..." : "Générer la grille"}
            </button>
            ...
            <div>
              <input
                ...
                onChange={(e) => {
                  const f = e.target.files?.[0];
                  if (f) handleUploadGrid(f);
                }}
              />
              <button
                onClick={() => fileInputGrid.current?.click()}
                disabled={uploadingGrid}
                className="flex items-center gap-2 rounded-lg border border-white/10 px-4 py-2 text-sm text-white/60 transition hover:bg-white/5 disabled:opacity-50"
              >
                {uploadingGrid ? (
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={() => {
                  setGridUrl(null);
                  setGridError(null);
                  if (fileInputGrid.current) fileInputGrid.current.value = "";
                }}
                className="flex items-center gap-2 rounded-lg border border-white/10 px-4 py-2 text-sm text-white/60 transition hover:bg-white/5"
              >
                <RefreshCw size={14} />
