...
export function CoursIcon({ className, style, ...rest }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div className={className} style={{ position: "relative", overflow: "hidden", borderRadius: 3, ...style }} {...rest}>
      {/* Page */}
      <div style={{ position: "absolute", inset: 0, background: "linear-gradient(170deg, #FEFCF8, #F0EBE0)", border: "1px solid rgba(201,168,76,0.5)", borderRadius: "inherit" }} />
      {/* Header */}
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: "20%", background: "#0B1A2E", borderRadius: "3px 3px 0 0", display: "flex", alignItems: "center", paddingLeft: "8%", paddingRight: "8%" }}>
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src="/logo-diploma-sante-white.svg" alt="" draggable={false} style={{ height: "55%", width: "auto", opacity: 0.9 }} />
      </div>
      {/* Gold line */}
      <div style={{ position: "absolute", top: "20%", left: 0, right: 0, height: 1, background: "linear-gradient(90deg, #C9A84C, #E3C286, #C9A84C)" }} />
      {/* Title block */}
      <div style={{ position: "absolute", top: "26%", left: "10%", right: "10%", height: "7%", borderRadius: 2, background: "rgba(11,26,46,0.06)" }} />
      <div style={{ position: "absolute", top: "36%", left: "15%", right: "15%", height: "5%", borderRadius: 2, background: "rgba(201,168,76,0.15)", border: "0.5px solid rgba(201,168,76,0.2)" }} />
      {/* Separator */}
      <div style={{ position: "absolute", top: "45%", left: "10%", right: "10%", height: 1, background: "linear-gradient(90deg, transparent, rgba(201,168,76,0.35), transparent)" }} />
      {/* Two columns */}
      <div style={{ position: "absolute", top: "49%", left: "7%", width: "40%", height: "35%", borderRadius: 2, border: "0.5px solid rgba(11,26,46,0.07)" }} />
      <div style={{ position: "absolute", top: "49%", right: "7%", width: "40%", height: "35%", borderRadius: 2, border: "0.5px solid rgba(11,26,46,0.07)" }} />
      {/* Lines left */}
      {[0,1,2].map(i => <div key={`l${i}`} style={{ position: "absolute", top: `${54 + i * 7}%`, left: "10%", width: `${24 - i * 5}%`, height: "2%", borderRadius: 1, background: `rgba(11,26,46,${0.07 - i * 0.02})` }} />)}
      {/* Lines right */}
      {[0,1,2].map(i => <div key={`r${i}`} style={{ position: "absolute", top: `${54 + i * 7}%`, right: "10%", width: `${24 - i * 5}%`, height: "2%", borderRadius: 1, background: `rgba(11,26,46,${0.07 - i * 0.02})` }} />)}
      {/* Bottom */}
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: "7%", background: "rgba(11,26,46,0.03)" }} />
    </div>
  );
}
