...
export default async function ExamenDetailPage({ params }: { params: Promise<{ examenId: string }> }) {
  ...
  const [examenRes, allDossiersRes, groupesRes, filieresRes, matieresRes, exGroupesRes] = await Promise.all([
    supabase
      .from("examens")
      .select("*, examens_series(*, series:series(id, name, type, matiere_id, timed, duration_minutes, description, cours_id, score_definitif, visible, annee, created_at, updated_at))")
      .eq("id", examenId)
      .single(),
